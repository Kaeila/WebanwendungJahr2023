...
public abstract class BeanTemplate<T extends ModelTemplate, D extends GenericDao> implements Serializable {
    ...
    public T getItem() {
        if (item == null) {
            item = (T) getDao().getNewEntityInstance();
        }
        return item;
    }
    ...
    public void setErrorMessage(String header, String msg) {
        setMessage(null, FacesMessage.SEVERITY_ERROR, header, msg);
    }

    public void setInfoMessage(String header, String msg) {
        setMessage(null, FacesMessage.SEVERITY_INFO, header, msg);
    }

    public void updateContentForm() {
        PrimeFaces.current().ajax().update(":contentForm");
    }

    public void updateMessageBox() {
        PrimeFaces.current().ajax().update(":messageBox");
    }
}
